...
for i in range(100):
    ...
    for j in range(50000):
        ...
        output = np.dot(X_train[loc], weights)
        activation = np.argmax(output+biases)

        valTrue = y_train[loc]
        valPred = activation

        if(valTrue != valPred):
            weights[:,valTrue] += a*X_train[loc]
            weights[:,valPred] -= a*X_train[loc]
 
            biases[valTrue] += a
            biases[valPred] -= a

    output = np.dot(X_train, weights)
    activation = np.argmax(output+biases, axis=1)
    print(np.sum(y_train == activation)/50000)
